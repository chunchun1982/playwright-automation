...
- Location: tests\AuctionLoginpage.spec.ts:69:9
...
> 102 |             await expect(page.locator('text=Inserted successfully')).toBeVisible();
  ...
  110 |         severity('normal');
  111 |         description('Verifies that the user can navigate to the Bidder Master page.');
  112 | 
  113 |         const auctionLoginPage = new AuctionLoginPage(page);
  114 | 
  115 |         await test.step('Navigate to Bidder Master', async () => {
  116 |             await auctionLoginPage.navigateToBidderMaster();
  117 |         });
  118 | 
  119 |         await test.step('Verify URL contains /Bidders', async () => {
  120 |             await expect(page).toHaveURL(/Bidders/);
  121 |         });
  122 |     });
  123 | 
  124 |     // ─────────────────────────────────────────────────────────────────
  125 |     test('Add Bidder', async () => {
  126 |         feature('Bidder Master');
  127 |         story('Add Bidder');
  128 |         severity('critical');
  129 |         description('Verifies that a new bidder can be added and linked to an existing client.');
  130 | 
  131 |         const addBidderPage = new AddBidderPage(page);
  132 | 
  133 |         await test.step('Click Add Bidder button', async () => {
  134 |             await addBidderPage.clickAddBidder();
  135 |         });
  136 | 
  137 |         await test.step('Fill in bidder details', async () => {
  138 |             await addBidderPage.addBidderDetails(
  139 |                 'Forward Auction',
  140 |                 'Kata Automation Company',
  141 |                 '123 Test St',
  142 |                 'AABCR1742E',
  143 |                 'India',
  144 |                 'Maharashtra',
  ...
  149 |                 '1234567890',
  150 |                 '9876543210',
  151 |                 'rakesh.singh@test.com',
  152 |                 'rakesh.singh'
  153 |             );
  154 |         });
  155 | 
  156 |         await test.step('Click Save', async () => {
  157 |             await addBidderPage.clickSave();
  158 |         });
  159 |     });
  160 | 
  161 | });
